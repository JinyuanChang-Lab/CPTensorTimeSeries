...
Rolling_fun2 <- function(x,
  ...
  out_na <- list(
    ...
    longrun_var_iter = c(h1 = NA_real_, h2 = NA_real_)
  )
  ...
  for (trying_time in seq_len(max_try)) {
    ...
    longrun_var_h1 <- tryCatch(
      cov.aij.debias.iter.longrun.est(
        h = h1,
        i = place.iter,
        j = j,
        A = A.iter,
...
Rolling_fun2_vec <- function(x,
  ...
  c(
    ...
    longrun_var_h1 = res$longrun_var_iter["h1"],
    longrun_var_h2 = res$longrun_var_iter["h2"]
  )
}
  ...
  for (factor.loading.num in factor.loading_tol) {
    ...
    for (delta.fl in delta.fl_tol) {
      ...
      for (ALPHA in ALPHA_tol) {
        ...
        rmse_h1_plugin_tol <- c()
        rmse_h2_plugin_tol <- c()
        rmse_h1_longrun_tol <- c()
        rmse_h2_longrun_tol <- c()
        ...
        for (n in n_tol) {
          ...
          if (length(idx) != 1) {
            rmse_h1_plugin_tol <- c(rmse_h1_plugin_tol, NA_real_)
            rmse_h2_plugin_tol <- c(rmse_h2_plugin_tol, NA_real_)
            rmse_h1_longrun_tol <- c(rmse_h1_longrun_tol, NA_real_)
            rmse_h2_longrun_tol <- c(rmse_h2_longrun_tol, NA_real_)
            next
          }
          ...
          rmse_h1_longrun <- sqrt(
            mean(
              (result_i$true_var_h1 - result_i$longrun_var_h1)^2,
              na.rm = TRUE
            )
          )

          rmse_h2_longrun <- sqrt(
            mean(
              (result_i$true_var_h2 - result_i$longrun_var_h2)^2,
              na.rm = TRUE
            )
          )

          rmse_h1_plugin_tol <- c(rmse_h1_plugin_tol, rmse_h1_plugin)
          rmse_h2_plugin_tol <- c(rmse_h2_plugin_tol, rmse_h2_plugin)
          rmse_h1_longrun_tol <- c(rmse_h1_longrun_tol, rmse_h1_longrun)
          rmse_h2_longrun_tol <- c(rmse_h2_longrun_tol, rmse_h2_longrun)
        }
        ...
        rmse_iter_tol_name <- cbind(
          ...
          delta.fl = delta.fl,
          ALPHA = ALPHA,
          estimator = c("plug-in", "plug-in", "long-run", "long-run"),
          h = c("h1", "h2", "h1", "h2"),
          rmse_iter_tol
        )
