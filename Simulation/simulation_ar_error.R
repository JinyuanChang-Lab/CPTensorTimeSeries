...
Rolling_fun2 <- function(x, n, ALPHA, delta.fl, factor.loading, factor.corr, seed_num) {
  ...
  data <- DGP.TCP(
    ...
    alpha = ALPHA, delta = delta.fl, par_E = 1, heavytail = FALSE,
    error.ar = 0.3
  )
...
#Rolling_fun2(x = 1, n, ALPHA, delta.fl, factor.loading, factor.corr, seed_num=1)
...
direction = paste0("simulation_estimation_ARerror_",format(Sys.time(), "%Y%m%d-%H-%M-%S"),".RData")
...
write.csv(con_tol, "table_rho_Ahat_A_ARerror.csv", row.names = FALSE)
